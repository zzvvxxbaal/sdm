      ...
      {error ? (
        ...
        <ul className="space-y-2">
          {songs.map((song) => {
            ...
            const className =
              "flex items-center gap-3 rounded-xl border border-[#e5e5e5] p-2.5 dark:border-[#2c2c2e]";
            return (
              <li key={song.id}>
                {song.youtubeUrl ? (
                  <a
                    ...
                    className={`${className} transition-colors hover:bg-[#fafafa] dark:hover:bg-[#262626]`}
                  >
                    {content}
                  </a>
                ) : (
