      ...
      <SectionCard title="이번 주 일정">
        {error ? (
          ...
          <EmptyState
            icon={CalendarDays}
            title="이번 주 일정이 없습니다"
            description="등록된 예배나 모임이 아직 없습니다."
          />
        ) : (
          <ul className="space-y-2.5">
            {events.map((event) => {
              ...
              return (
                <li
                  key={event.id}
                  className="flex items-start gap-3 rounded-xl border border-[#e5e5e5] p-3 dark:border-[#2c2c2e]"
                >
                  ...
                  <div className="min-w-0 flex-1">
                    ...
                    <p className="mt-0.5 text-xs font-medium text-[#2563EB]">
                      {formatEventWhen(event.startDate, event.isAllDay)}
                    </p>
                    {event.location && (
